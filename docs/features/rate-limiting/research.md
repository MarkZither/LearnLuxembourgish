...
| Global token bleed from exhausted-identity callers (SEC-003) | Low for v1 | A per-identity-exhausted caller still consumes one global token per rejected request (global check runs first per FR-012). At current scale (low traffic, 60-minute windows) this is not exploitable in practice. Accept for v1; revisit if subscriber base grows to the point where a single user can meaningfully drain the global authenticated pool (50 tokens/hr). |
